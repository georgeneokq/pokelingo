...
public class PetalburgGymVocabWeatherTrainer extends NPC implements Opponent, EventManager.Subscriber {
    ...
    public PetalburgGymVocabWeatherTrainer(float width, float height, TextureAtlas textureAtlas, Texture battleSprite) {
        super(new Rectangle(0, 0, width, height), "Petalburg trainer Ryo", textureAtlas);
    ...
    private void loadAndShuffleQuestions() {
        QuestionFilter[] questionFilters = new QuestionFilter[] {
                new QuestionFilter((question) -> question.getName().startsWith("vocabulary_weather"), 0),
        };
