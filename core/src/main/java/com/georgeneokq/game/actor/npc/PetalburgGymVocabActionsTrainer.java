...
public class PetalburgGymVocabActionsTrainer extends NPC implements Opponent, EventManager.Subscriber {
    ...
    public PetalburgGymVocabActionsTrainer(float width, float height, TextureAtlas textureAtlas, Texture battleSprite) {
        super(new Rectangle(0, 0, width, height), "Petalburg trainer Kita", textureAtlas);
    ...
    private void loadAndShuffleQuestions() {
        QuestionFilter[] questionFilters = new QuestionFilter[] {
                new QuestionFilter((question) -> question.getName().startsWith("vocabulary_actions"), 0),
        };
