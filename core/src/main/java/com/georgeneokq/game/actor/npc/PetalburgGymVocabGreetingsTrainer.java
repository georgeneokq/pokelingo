...
public class PetalburgGymVocabGreetingsTrainer extends NPC implements Opponent, EventManager.Subscriber {
    ...
    public PetalburgGymVocabGreetingsTrainer(float width, float height, TextureAtlas textureAtlas, Texture battleSprite) {
        super(new Rectangle(0, 0, width, height), "Petalburg trainer Bocchi", textureAtlas);
    ...
    private void loadAndShuffleQuestions() {
        QuestionFilter[] questionFilters = new QuestionFilter[] {
                new QuestionFilter((question) -> question.getName().startsWith("vocabulary_greetings"), 0),
        };
