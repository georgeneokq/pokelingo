...
public class PetalburgGymKatakanaTrainer extends NPC implements Opponent, EventManager.Subscriber {
    ...
    public PetalburgGymKatakanaTrainer(float width, float height, TextureAtlas textureAtlas, Texture battleSprite) {
        super(new Rectangle(0, 0, width, height), "Petalburg trainer Kyosuke", textureAtlas);
    ...
    private void loadAndShuffleQuestions() {
        QuestionFilter[] questionFilters = new QuestionFilter[] {
                new QuestionFilter((question) -> question.getName().startsWith("katakana_"), 0),
        };
