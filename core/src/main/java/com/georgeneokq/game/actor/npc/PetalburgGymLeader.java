...
public class PetalburgGymLeader extends NPC implements Opponent, EventManager.Subscriber {
    ...
    public PetalburgGymLeader(float width, float height, TextureAtlas textureAtlas, Texture battleSprite) {
        super(new Rectangle(0, 0, width, height), "Gym Leader", textureAtlas);
    ...
    private void loadAndShuffleQuestions() {
        QuestionFilter[] questionFilters = new QuestionFilter[] {
            new QuestionFilter((question) -> question.getName().startsWith("hiragana_"), 5),
            new QuestionFilter((question) -> question.getName().startsWith("katakana_"), 5),
            new QuestionFilter((question) -> question.getName().startsWith("vocabulary_weather"), 5),
            new QuestionFilter((question) -> question.getName().startsWith("vocabulary_greetings"), 5),
            new QuestionFilter((question) -> question.getName().startsWith("vocabulary_actions"), 5),
            new QuestionFilter((question) -> question.getName().startsWith("sentence_forming"), 5),
            new QuestionFilter((question) -> question.getName().startsWith("particles"), 5),
        };
